...
local spin_speed = 999

Hooks:PostHook(GageAssignmentBase, "init", __NameIds("init"), function(self, __unit, ...)
	self._unit:set_local_position(self._unit:local_position() + Vector3(5, 5, flow_offset))
...
function GageAssignmentBase:update(__unit, __t, __dt, ...)
	if self._unit and alive(self._unit) and type(__t) == "number" and type(__dt) == "number" then
		self._unit:set_local_rotation(self._unit:local_rotation() * Rotation(Vector3(5, 5, 1), __dt * spin_speed))
		if self[old_pos] then
			self._unit:set_local_position(self[old_pos] + Vector3(5, 5, math.sin(__t*flow_offset_speed) * flow_offset_moving))
		end
	end
end